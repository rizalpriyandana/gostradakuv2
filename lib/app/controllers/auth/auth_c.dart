...
import 'package:gostradav2/app/data/models/login_m.dart';
...
class AuthController extends GetxController {
  ...
  resetpassword(String nim) async {
    ...
    var response = await http.post(
        Uri.parse("https://siakadtest.strada.ac.id/mobile/lupa_password/generate"),
        body: dataBody);
    if (response.statusCode == 200) {
      var user = jsonDecode(response.body);
      if (user['error'] == true) {
  ...
  insertToken(String nim, String token) async {
    ...
    var response = await http.post(
        Uri.parse("https://siakadtest.strada.ac.id/mobile/token/save_token"),
        body: dataBody);
    if (response.statusCode == 200) {
      var user = jsonDecode(response.body);
      if (user['error'] == true) {
  ...
  deleteToken(String nim) async {
    ...
    var response = await http.post(
        Uri.parse("https://siakadtest.strada.ac.id/mobile/token/delete_token"),
        body: dataBody);
    if (response.statusCode == 200) {
      var user = jsonDecode(response.body);
      if (user['error'] == true) {
